...
function renderMarkdown(markdown = '') {
  ...
  for (const line of lines) {
    ...
    if (line.trimStart().startsWith('|')) {
      ...
      const rows = [line];
      for (const next of lines.slice(lines.indexOf(line) + 1)) {
        if (next.trimStart().startsWith('|')) rows.push(next);
        else break;
      }
      ...
      // Header row (skip the separator row which is rows[1])
      const dataRows = rows.filter((r, i) => i !== 1 || !/^[\s|:-]+$/.test(r));
      dataRows.forEach((row) => {
        const cells = row.split('|').filter(c => c.trim());
        const isHeader = cells.length > 0 && /#/.test(cells[0]); // heuristic for header
        table += '<tr>';
        cells.forEach((cell) => {
          const tag = isHeader || row === dataRows[0] ? 'th' : 'td';
          table += `<${tag}>${escapeHtmlLocal(cell.trim()).replace(/\*(.+?)\*/g, '<em>$1</em>').replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>')}</${tag}>`;
        });
        table += '</tr>';
      });
      table += '</table></div>';
      ...
      for (let skip = 0; skip < rows.length - 1; skip++) {
        lines[lines.indexOf(line) + skip] = '';
      }
      continue;
    }
...
function tagMarkup(tags = []) {
  return tags.map((tag) => `<span class="tag" data-action="tag-filter" data-value="${escapeHtml(tag)}" style="cursor:pointer">${escapeHtml(tag)}</span>`).join('');
}
...
function renderLeftRail() {
  ...
  const openModelReplies = modelReplies().filter((reply) => reply.status !== 'resolved').length;
  const pendingProposals = state.manifest.proposals.filter((proposal) => decisionFor(proposal) === 'proposed').length;
  const viewCounts = {
    dashboard: pendingProposals + openModelReplies,
...
function dashboardMetrics() {
  const pendingProposals = state.manifest.proposals.filter((proposal) => decisionFor(proposal) === 'proposed').length;
  const openReplies = modelReplies().filter((reply) => reply.status !== 'resolved').length;
  return [
    ...
    [pendingProposals + openReplies, 'Decisions needed'],
    [openReplies, 'Model replies'],
  ...
  return `
    <div class="view-frame document-view">
      <div class="document-intro">
        ...
        ${(() => { const open = (state.manifest.modelReplies || []).filter(r => r.status === 'open'); return open.length ? `<div class="model-reply-alert" style="background:var(--accent-wash);border:1px solid var(--accent);border-radius:4px;padding:0.6rem 1rem;margin-bottom:0.8rem;display:flex;gap:0.5rem;align-items:center;font-size:0.85rem"><span>💬</span><span><strong>${open.length} open model ${open.length === 1 ? 'reply' : 'replies'}</strong> — navigate to the <span class="tag" data-action="set-view" data-value="dashboard" style="cursor:pointer">Dashboard</span> to respond</span></div>` : ''; })()}
...
app.addEventListener('click', (event) => {
  ...
  if (action === 'set-view') setView(control.dataset.value);
  ...
  if (action === 'copy-ledger') {
    const textarea = document.querySelector('#change-ledger-text');
    if (textarea) { textarea.select(); navigator.clipboard.writeText(textarea.value).then(() => toast('Ledger copied')).catch(() => toast('Copy failed')); }
  }
